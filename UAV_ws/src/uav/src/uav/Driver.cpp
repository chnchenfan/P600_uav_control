...
{
    set_target_position_sub = nh.subscribe("/gi/set_pose/position",1,&Driver::set_target_position_callback,this);//位置信息设置订阅
    set_target_yaw_sub = nh.subscribe("/gi/set_pose/orientation",1,&Driver::set_target_yaw_callback,this);//偏航角订阅
    custom_activity_sub = nh.subscribe("/gi/set_activity/type",1,&Driver::custom_activity_callback,this);//接收到activity后对模式进行设置
