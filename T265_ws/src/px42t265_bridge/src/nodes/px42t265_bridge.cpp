...
px42t265_bridge::px42t265_bridge(const ros::NodeHandle& nh)
    : nh_(nh) {

  // initialize subscribers
  odom_sub_ = nh_.subscribe<const nav_msgs::Odometry&>(
      "/camera/odom/sample_throttled", 10, &px42t265_bridge::odomCallback, this);
  tof_sub_ = nh_.subscribe<const sensor_msgs::Range&>(
    "/tfmini_ros_node/TFmini", 10, &px42t265_bridge::tofCallback, this);
  // publishers
  mavros_odom_pub_ =
  ...
  worker_ = std::thread(&px42t265_bridge::publishSystemStatus, this);


};

px42t265_bridge::~px42t265_bridge() { }
...
void px42t265_bridge::odomCallback(const nav_msgs::Odometry& msg) {
...
void px42t265_bridge::tofCallback(const sensor_msgs::Range& msg) {

  output.pose.pose.position.z = msg.range;
}

void px42t265_bridge::publishSystemStatus(){
